"""Evaluate SUIM SPT against the fixed F4 comparator on development only."""
...
from scripts.train_suim_spt import FORMAT, build_model as build_spt  # noqa: E402


def main() -> None:
    parser = argparse.ArgumentParser(description=__doc__); parser.add_argument("--config", type=Path, default=ROOT / "configs/spt_seg_suim.yaml"); args = parser.parse_args(); config = yaml.safe_load(args.config.read_text(encoding="utf-8"))
    development = ROOT / str(config["data"]["development_csv"])
    if development.name != "risk_head_development.csv" or len(pd.read_csv(development)) != 231: raise PermissionError("SPT evaluation accepts only the frozen SUIM development role.")
    ...
    spt_path = ROOT / str(config["experiment"]["output_dir"]) / "formal/SPT/checkpoints/final.pt"; spt_payload = torch.load(spt_path, map_location=device, weights_only=False); required = {"checkpoint_format": FORMAT, "variant": "SPT", "epoch": int(config["training"]["epochs"]), "smoke": False}
    for key, expected in required.items():
        if spt_payload.get(key) != expected: raise ValueError(f"Invalid SPT checkpoint field {key}.")
    spt = build_spt(config, device); spt.load_state_dict(spt_payload["model_state_dict"]); spt.eval()
