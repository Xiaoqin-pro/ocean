...
def main() -> None:
    parser = argparse.ArgumentParser(description=__doc__); parser.add_argument("--config", type=Path, default=ROOT / "configs/sadr_fast.yaml"); parser.add_argument("--split", choices=("confirmation", "suim_official"), default="confirmation"); parser.add_argument("--allow-confirmation", action="store_true"); args = parser.parse_args(); config = yaml.safe_load(args.config.resolve().read_text(encoding="utf-8")); data = config["data"]
    ...
    device = torch.device("cuda"); base, front = build_model(config, device); payload = torch.load(ROOT / str(config["experiment"]["output_dir"]) / "formal/checkpoints/final.pt", map_location=device, weights_only=False); front.load_state_dict(payload["model_state_dict"]); gate = json.loads((ROOT / str(config["experiment"]["output_dir"]) / "quality_gate/gate.json").read_text(encoding="utf-8")); base.eval(); front.eval(); gated = QualityGateModel(base, front, gate).eval(); conditions = [item for item in load_conditions(ROOT / str(data["degradation_registry"])) if item.name in set(config["evaluation"]["conditions"])]
    ...
    result = pd.DataFrame(rows); output = ROOT / str(config["experiment"]["output_dir"]) / f"{args.split}_quality_gate_evaluation"; output.mkdir(parents=True, exist_ok=True); result.to_csv(output / "condition_metrics.csv", index=False); summary = {"split": args.split, "confirmation_evaluated": args.split == "confirmation", "official_suim_test_evaluated": args.split == "suim_official", "rows": len(result), "images": expected, "mean_miou": float(result.miou.mean()), "gate_source": "508-image calibration only"}; (output / "summary.json").write_text(json.dumps(summary, indent=2) + "\n", encoding="utf-8"); print(json.dumps(summary, indent=2), flush=True)
