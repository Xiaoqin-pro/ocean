"""Train the preregistered SCDI-Seg Gate-2 candidate on method_train only."""
...
from reliability.scdi_seg import SCDISegformer, rectifier_parameter_count, semantic_router_loss  # noqa: E402
...
FORMAT = "scdi_seg_gate2_v1"


def build_model(config: dict[str, Any], device: torch.device) -> SCDISegformer:
...
def checkpoint_payload(model, optimizer, scaler, *, epoch, config_path, source_path, history, smoke):
    return {
...
def main() -> None:
    ...
    if not torch.cuda.is_available():
        raise RuntimeError("SCDI Gate-2 requires CUDA.")
    ...
    output = ROOT / str(config["experiment"]["output_dir"]) / ("smoke" if args.smoke_batches else "formal") / "SCDI"
    history: list[dict[str, float]] = []
    source_path = ROOT / str(config["model"]["source_checkpoint"])
    model.train()
    for epoch in range(1, epochs + 1):
        dataset.set_epoch(epoch)
        totals = {"ce": 0.0, "tangent": 0.0, "identity": 0.0, "router": 0.0, "class_scale_pairs": 0.0}
        batches = 0
        for batch_index, batch in enumerate(loader):
            ...
            for key in ("clean", "s1", "s2", "s3"):
                ...
                with torch.autocast(device_type="cuda", dtype=torch.float16, enabled=bool(training["amp"])):
                    ...
                    ce = functional.cross_entropy(logits, labels, ignore_index=255) / 4.0
                    ...
                    if key == "clean":
                        ...
                        tangent, pairs = semantic_tangent_loss(
                            clean_reference, result.raw_features, result.residuals, labels,
                            num_classes=int(config["model"]["num_classes"]), minimum_pixels=int(training["tangent_minimum_pixels"]),
                        )
                    loss = (
                        ce + float(training["tangent_weight"]) * tangent
                        + float(training["clean_identity_weight"]) * identity
            ...
            if batch_index % 25 == 0:
                print(
                    f"SCDI epoch={epoch} batch={batch_index}/{len(loader)} ce={totals['ce']/batches:.4f} "
                    f"tangent={totals['tangent']/batches:.4f} router={totals['router']/batches:.4f} "
                    f"pairs={totals['class_scale_pairs']/batches:.1f}", flush=True,
                )
        if batches == 0:
