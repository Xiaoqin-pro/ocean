...
from reliability.sadr_seg import FrequencySADRFrontEnd, RoutedSADRFrontEnd, SADRFrontEnd, confidence_distillation, gradient_reconstruction_loss, segmentation_loss, semantic_feature_consistency, semantic_view_consistency  # noqa: E402
...
def main() -> None:
    ...
    for epoch in range(1, epochs + 1):
        dataset.set_epoch(epoch); front.train(); totals = {key: 0.0 for key in ("segmentation", "reconstruction", "gradient", "identity", "distillation", "feature", "consistency", "residual")}; batches = 0
        for batch_index, batch in enumerate(loader):
            ...
            with torch.autocast(device_type="cuda", dtype=torch.float16, enabled=bool(training["amp"])):
                ...
                loss = float(training["segmentation_weight"]) * segmentation + float(training["reconstruction_weight"]) * reconstruction + gradient_weight * gradient + float(training["identity_weight"]) * identity + float(training["distillation_weight"]) * distillation + feature_weight * feature + consistency_weight * consistency + float(training["residual_weight"]) * residual_value + float(training.get("router_weight", 0.0)) * router_loss
            scaler.scale(loss).backward(); scaler.step(optimizer); scaler.update();
            for key, value in (("segmentation", segmentation), ("reconstruction", reconstruction), ("gradient", gradient), ("identity", identity), ("distillation", distillation), ("feature", feature), ("consistency", consistency), ("residual", residual_value)): totals[key] += float(value.detach())
            batches += 1
