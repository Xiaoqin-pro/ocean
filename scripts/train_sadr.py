...
from reliability.sadr_seg import FrequencySADRFrontEnd, RoutedSADRFrontEnd, SADRFrontEnd, confidence_distillation, segmentation_loss  # noqa: E402
    ...
    for epoch in range(1, epochs + 1):
        ...
        for batch_index, batch in enumerate(loader):
            ...
            with torch.autocast(device_type="cuda", dtype=torch.float16, enabled=bool(training["amp"])):
                routed = bool(training.get("routed", False)); degraded_output = front(degraded); clean_output = front(clean); restored, residual = degraded_output[:2]; restored_clean, clean_residual = clean_output[:2]; student = base(pixel_values=restored).logits; teacher_rep = teacher.repeat(3, 1, 1, 1); segmentation = segmentation_loss(student, repeated_labels); reconstruction = (restored - clean_repeated).abs().mean(); identity = clean_residual.abs().mean(); distillation = confidence_distillation(student, teacher_rep); residual_value = residual.float().pow(2).mean() + clean_residual.float().pow(2).mean(); router = degraded_output[2] if routed else None; router_loss = torch.nn.functional.cross_entropy(router, family_ids) if router is not None else segmentation * 0.0
                ...
                loss = float(training["segmentation_weight"]) * segmentation + float(training["reconstruction_weight"]) * reconstruction + float(training["identity_weight"]) * identity + float(training["distillation_weight"]) * distillation + float(training["residual_weight"]) * residual_value + float(training.get("router_weight", 0.0)) * router_loss
            ...
            del clean, labels, degraded, family_ids, repeated_labels, clean_repeated, teacher, restored, residual, restored_clean, clean_residual, student, teacher_rep, loss, degraded_output, clean_output
