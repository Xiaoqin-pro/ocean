...
from reliability.sadr_seg import FrequencySADRFrontEnd, RoutedSADRFrontEnd, SADRFrontEnd, confidence_distillation, segmentation_loss, semantic_feature_consistency, semantic_view_consistency  # noqa: E402
...
def main() -> None:
    ...
    for epoch in range(1, epochs + 1):
        dataset.set_epoch(epoch); front.train(); totals = {key: 0.0 for key in ("segmentation", "reconstruction", "identity", "distillation", "feature", "consistency", "residual")}; batches = 0
        for batch_index, batch in enumerate(loader):
            ...
            for key, value in (("segmentation", segmentation), ("reconstruction", reconstruction), ("identity", identity), ("distillation", distillation), ("feature", feature), ("consistency", consistency), ("residual", residual_value)): totals[key] += float(value.detach())
            batches += 1
