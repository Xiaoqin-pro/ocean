"""Train UTD on the frozen UIIS train role."""
...
from scripts.train_uiis_spt import build_model, sha256, atomic_save  # noqa: E402

FORMAT = "uiis_utd_v1"


def main() -> None:
    parser = argparse.ArgumentParser(description=__doc__); parser.add_argument("--config", type=Path, default=ROOT / "configs/uiis_utd.yaml"); parser.add_argument("--smoke-batches", type=int, default=0); args = parser.parse_args(); config_path = args.config.resolve(); config = yaml.safe_load(config_path.read_text(encoding="utf-8"))
    ...
    if not torch.cuda.is_available(): raise RuntimeError("UIIS UTD requires CUDA.")
    device = torch.device("cuda"); training = config["training"]; dataset = UIISTrajectoryDataset(ROOT / str(config["data"]["train_csv"]), ROOT / str(config["data"]["degradation_registry"]), int(config["data"]["image_size"]))
    ...
    epochs = 1 if args.smoke_batches else int(training["epochs"]); output = ROOT / str(config["experiment"]["output_dir"]) / ("smoke" if args.smoke_batches else "formal"); source_path = ROOT / str(config["model"]["source_checkpoint"]); history = []; model.train()
        ...
        for batch_index, batch in enumerate(loader):
            ...
            for view_index, key in enumerate(("clean", "s1", "s2", "s3")):
                ...
                with torch.autocast(device_type="cuda", dtype=torch.float16, enabled=bool(training["amp"])):
                    ...
                    if key == "clean": identity = clean_identity_loss(result.raw_features, result.residuals); clean_reference = tuple(item.detach() for item in result.raw_features); teacher_logits = logits.detach()
                    else:
            ...
            if batch_index % 50 == 0: print(f"UTD epoch={epoch} batch={batch_index}/{len(loader)} ce={totals['ce']/batches:.4f} proto={totals['prototype']/batches:.4f} distill={totals['distillation']/batches:.4f}", flush=True)
