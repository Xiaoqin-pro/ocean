...
def evaluate_model(model: torch.nn.Module, root: Path, enhanced: bool, device: torch.device) -> dict:
    transform = A.Compose([A.Resize(384, 384), A.Normalize(mean=IMAGENET_MEAN, std=IMAGENET_STD, max_pixel_value=255.0), ToTensorV2()]); confusion = np.zeros((8, 8), dtype=np.int64); count = 0
    for mask_path in sorted((root / "mask").glob("*.png")):
        ...
        transformed = transform(image=image_array, mask=mapped); pixels = transformed["image"].unsqueeze(0).float().to(device); target = transformed["mask"].long().unsqueeze(0).to(device)
        with torch.no_grad(), torch.autocast(device_type="cuda", dtype=torch.float16): logits = model(pixel_values=pixels).logits
        pred = functional.interpolate(logits.float(), size=target.shape[-2:], mode="bilinear", align_corners=False).argmax(1); valid = target.ne(255); indices = target[valid] * 8 + pred[valid]; confusion += torch.bincount(indices, minlength=64).reshape(8, 8).cpu().numpy(); count += 1
...
def main() -> None:
    parser = argparse.ArgumentParser(description=__doc__); parser.add_argument("--root", type=Path, default=ROOT / "data/uvmulti/val/video108"); args = parser.parse_args();
    if not torch.cuda.is_available(): raise RuntimeError("CUDA is required.")
    device = torch.device("cuda"); uiis_config = yaml.safe_load((ROOT / "configs/uiis_scdi_replication.yaml").read_text(encoding="utf-8")); baseline = build_uiis_f4(uiis_config, "F4", device).eval(); sadr1 = load_sadr("configs/sadr_long.yaml", device); sadr2 = load_sadr("configs/sadr_seed2.yaml", device); rows = []
    for view, enhanced in (("raw", False), ("enhanced", True)):
        for name, model in (("UIIS-F4", baseline), ("SADR-seed1", sadr1), ("SADR-seed2", sadr2)):
            metric = evaluate_model(model, args.root, enhanced, device); rows.append({"view": view, "variant": name, **metric}); print(f"{view}/{name}: common_mIoU={metric['miou_common']:.4f}", flush=True)
    output = ROOT / "outputs/sadr_uvmulti_validation"; output.mkdir(parents=True, exist_ok=True); frame = pd.DataFrame(rows); frame.to_csv(output / "metrics.csv", index=False); (output / "summary.json").write_text(json.dumps({"video": args.root.name, "frames": 26, "class_mapping": UVMULTI_TO_SUIM, "rows": rows}, indent=2) + "\n", encoding="utf-8")
