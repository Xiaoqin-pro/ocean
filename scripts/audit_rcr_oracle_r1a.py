...
def main():
 ap=argparse.ArgumentParser();ap.add_argument('--config',type=Path,default=ROOT/'configs'/'rcr_oracle_r1a.yaml');a=ap.parse_args();c=yaml.safe_load(a.config.read_text(encoding='utf8'))
 ...
 out=ROOT/c['experiment']['output_dir'];scene=pd.DataFrame(rows);assert len(scene)==13*231*3 and not scene.duplicated(['sample_id','condition','region']).any(); ar=[]
 ...
 aggregate=pd.DataFrame(ar);boot=bootstrap(scene,int(c['experiment']['seed']));full=aggregate[aggregate.region=='full'];families=full[full.degradation_type!='clean'].groupby('degradation_type').delta_miou.mean();b=boot[boot.region=='full'].iloc[0];repair=scene[scene.region=='full'].repaired_pixels.sum();damage=scene[scene.region=='full'].damaged_pixels.sum();delta=float(full.delta_miou.mean());context=float(full.mean_context_area_ratio.mean());checks={'miou':delta>=c['gate_r1a']['macro_miou_improvement_min'],'oracle_recovery':delta/c['metrics']['r0_oracle_macro_miou_gain']>=c['gate_r1a']['oracle_gain_recovery_fraction_min'],'families':int((families>0).sum())>=c['gate_r1a']['positive_family_count_min'],'bootstrap':float(b.ci95_low)>0,'damage':damage/max(repair,1)<c['gate_r1a']['damage_to_repair_ratio_max'],'context':context<=c['gate_r1a']['mean_context_area_ratio_max']};decision={'gate':'R1a_uncompressed_gt_crop_local_refinement','checks':checks,'passes':all(checks.values()),'macro_miou_delta':delta,'oracle_gain_recovery_fraction':delta/c['metrics']['r0_oracle_macro_miou_gain'],'positive_families':int((families>0).sum()),'ci95_low':float(b.ci95_low),'damage_to_repair_ratio':damage/max(repair,1),'mean_context_area_ratio':context,'r1b_authorized':all(checks.values()),'calibration_evaluated':False,'validation_evaluated':False,'official_suim_test_evaluated':False}
