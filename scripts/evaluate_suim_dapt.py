"""Evaluate DAPT on the frozen SUIM development role."""
...
from scripts.train_suim_dapt import FORMAT, build_model as build_dapt  # noqa: E402
from scripts.train_dts_seg_gate0 import build_model as build_f4  # noqa: E402


def main() -> None:
    parser = argparse.ArgumentParser(description=__doc__); parser.add_argument("--config", type=Path, default=ROOT / "configs/dapt_seg_suim.yaml"); args = parser.parse_args(); config = yaml.safe_load(args.config.read_text(encoding="utf-8")); development = ROOT / str(config["data"]["development_csv"])
    ...
    dapt_path = ROOT / str(config["experiment"]["output_dir"]) / "formal/DAPT/checkpoints/final.pt"; dapt_payload = torch.load(dapt_path, map_location=device, weights_only=False); required = {"checkpoint_format": FORMAT, "variant": "DAPT", "epoch": int(config["training"]["epochs"]), "smoke": False}
    for key, expected in required.items():
        if dapt_payload.get(key) != expected: raise ValueError(f"Invalid DAPT checkpoint field {key}.")
    dapt = build_dapt(config, device); dapt.load_state_dict(dapt_payload["model_state_dict"]); dapt.eval()
    for variant, model in (("F4", f4), ("DAPT", dapt)):
